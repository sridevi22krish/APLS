...
/* MGS Custo start*/
			DomainObject childObj = new DomainObject(selectedId);
			DomainObject parentObj = new DomainObject(selPartObjectId);
			
 						String strParentType = parentObj.getInfo(context, "type");
 						String strChildType = childObj.getInfo(context, "type");
 						String strRelType = "relationship_EBOM";
 						if("AT_C_LOGICAL_NODE".equals(strParentType)){
 							if("AT_C_LOGICAL_NODE".equals(strChildType)){
 								strRelType = "relationship_EBOM";
 							} else if("AT_C_CONFIGURATION_ITEM".equals(strChildType)){
 								strRelType = "relationship_EBOM";
 							}
 						} else if ("AT_C_CONFIGURATION_ITEM".equals(strParentType)){
 							if("AT_C_EXPECTED_PRODUCT".equals(strChildType)){
 								strRelType = "relationship_EBOM";
 							}
 						} else if ("AT_C_EXPECTED_PRODUCT".equals(strParentType)){
							StringBuffer sbTypes = new StringBuffer();
							sbTypes.append(PropertyUtil.getSchemaProperty(context, "type_AT_C_DESIGN_PART")).append(",").append(PropertyUtil.getSchemaProperty(context, "type_AT_C_COS")).append(",").append(PropertyUtil.getSchemaProperty(context, "type_AT_C_STANDARD_PART"));
 							if(sbTypes.toString().equals(strChildType)){
							//if("MGS_Solution".equals(strChildType)){
 								strRelType = "relationship_EBOM";
 							}
 						}
 						
 						
            sUOMForMarkUp = childObj.getInfo(context, EngineeringConstants.SELECT_ATTRIBUTE_UNITOFMEASURE); //UOM Management
			...
			if(currentState.equalsIgnoreCase(DomainConstants.STATE_PART_PRELIMINARY) && "view".equalsIgnoreCase(tablemode)){
				...
				if(status){
					dr =DomainRelationship.connect(context, domObj, PropertyUtil.getSchemaProperty(context, strRelType), DomainObject.newInstance(context, selectedId));
					dr.setAttributeValues(context, hmRelAttributesMap);
				}
				sbOut.append("<item oid=\"").append(selectedId).append("\" pid=\"").append(selPartObjectId).append("\" relId=\"").append(dr.getName()).append("\" relType=\"" +strRelType +"\">");
	            ...
	            sbOut.append("<column name=\"Usage\" edited=\"true\" actual=\"Standard\">"+strStandard+"</column>");            
	            sbOut.append("<column name=\"AT_C_Quantity\" edited=\"true\">1.0</column>");
			}
			else{
				sbOut.append("<item oid=\"").append(selectedId).append("\" pid=\"").append(selPartObjectId).append("\" relType=\"" +strRelType +"\">");
				sbOut.append("<column name=\"Find Number\" edited=\"true\">"+ FNvalue +"</column>").append("<column name=\"Reference Designator\" edited=\"true\"></column>");
	            sbOut.append("<column name=\"Usage\" edited=\"true\" actual=\"Standard\">"+strStandard+"</column>");            
	            sbOut.append("<column name=\"AT_C_Quantity\" edited=\"true\">1.0</column>");
			}
