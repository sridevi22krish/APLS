...
emxEditableTable.prototype.init = function __init(){
    ...
    */
	...
	var showTableHeader = document.getElementById("showTableHeader");
    if(showTableHeader){
       showTableHeader=showTableHeader.value;
    }
    else{
    ...
    addOrHideHeaderItems(showTableHeader);
...
//	toggleToolbarItems(displayView);
...
//End:For Enhanced Calendar Control:AEF:nr2:20-11-09
		...
		if(objColumn.getAttribute("name") && objColumn.getAttribute("name") == "Name" && configuredTableName && 
			(configuredTableName.indexOf("ENCEBOMIndentedSummarySB") > -1 || configuredTableName.indexOf("PUEUEBOMIndentedSummary") > -1)){
			appendParams+="&excludeOIDprogram=emxUnresolvedPart:at_excludeRecursiveOIDAddExistingAndTemplate";
		}

                appendParams+="&suiteKey="+suiteKey;
...
function updateText(objElm,obj, rowId){
    ...
    if(edited && validated){
        ...
        if(eleminnerHTML.indexOf('iconStatusChanged') < 0 && status !== "add" && status !== "new" && status !== "lookup") {
			var toolTip=emxUIConstants.STR_SB_EDIT_TOOLTIP;
            elem.innerHTML = eleminnerHTML + '<img height=\"16\" align=\"middle\" src=\"images/iconStatusChanged.gif\" title="'+emxUIConstants.STR_SB_EDIT_TOOLTIP+'" />&#160;';        }
        }else{
            bFullTreeDrawn = false;
            RefreshView();
        }
		...
		if(!(showApply == "false")){
   		...
   		exitEditDialog.getContent().getElements('.btn').forEach(function (element) {
								if(element.id=="save"){
									...
									applyEdits();switchToViewMode();exitEditDialog.hide();
									});
...
function exportAllData(){
	turnOnProgress();
    setTimeout(function(){exportAllData2()},25);
}

function exportAllData2(){
	var r = confirm(emxUIConstants.EXPALL_CONFIRM_MSG);
	if (r == true) {
		var winName = "listHidden";
		var outputFormat = "CSV";
		var subHeader = getRequestSetting('subHeader');
	    var strURL = "emxFreezePaneExportAll.jsp?exportAllData=true&exportFormat=" + outputFormat + "&subHeader=" + subHeader+ "&sbMode="+editableTable.mode;
...
function link(colNum,oid,relId,parentId,linkName){
    ...
    if (target === "content" && href.indexOf("emxTree.jsp") > -1) {
        ...
        if(!getTopWindow().setUWAPref && getTopWindow().getWindowOpener()){
          ...
          } else if(getTopWindow().setUWAPref){
...
function validateDrop(dragData, dropid) {
	...
	if (!found) {
		var strException = emxUIConstants.STR_OBJECT_TYPE_NOT_ALLOWED;
		throw strException.replace("$<type>", unFoundType);
	}
	...
	if (dropid) {
		...
		for (var j = 0; j < dragData.objects.length; j++) {
			...
			if (matchedRows.length > 0) {
				var strException = emxUIConstants.STR_NO_PARENT_AS_CHILD;
				throw strException;
				break;
			}
		}
...
FreezePaneUtils.getCurrentChannelName = function _getCurrentChannelName(){
    var returnText="";
	if(portalMode == "true"){
		if(parent.$(".tab-active").length > 1 ||(parent.$(".tab-active").length ==0)){
			returnText = document.title;
	    }else{
			returnText = parent.$(".tab-active")[0].title;
		}
	}
	return returnText;
}
...
function getTDValueForSelectList(objTD){
    return objTD.firstChild ? objTD.firstChild.wholeText:"";
}
...
function showDetailsView(persist){
    ...
    if(firstTime){
        ...
        try{
			...
			sortDirection = decodeURIComponent(sortDirection);
            ...
            if(sortColumnName != null && sortColumnName != '' && sortColumnName != 'null' && sortColumnName != 'undefined') {
				sortColumnName = decodeURIComponent(sortColumnName);
