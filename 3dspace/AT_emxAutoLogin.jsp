...
if (session.getAttribute("isOpened") == null){
 ...
 session.setAttribute("isOpened", "true");
...
//if (islogged)	
    ...
    String targetPage = FrameworkProperties.getProperty("emxLogin.FrameworkTarget");
	 ...
	 if(objectId != null && !objectId.isEmpty()){
    	targetPage = "common/AT_emxFileDownLoad.jsp?objectId="+objectId+"&fileName="+XSSUtil.encodeForURL(fileName)+"&fileFormat="+fileFormat;
    }
...
%>
	...
	String fileFormat = request.getParameter("fileFormat");
    if(objectId != null && !objectId.isEmpty()){
		FrameworkTarget = "common/AT_emxFileDownLoad.jsp?objectId="+objectId+"&fileName="+XSSUtil.encodeForURL(fileName)+"&fileFormat="+fileFormat;
	    forwardURL = "common/AT_emxFileDownLoad.jsp?objectId="+objectId+"&fileName="+XSSUtil.encodeForURL(fileName)+"&fileFormat="+fileFormat;
	}
      ...
      //this should close all popup windows if opened in a chain
       if ((window.opener != null) && (window.opener != "undefined")){
        try
        {
            ...
            var openerProtocol = window.opener.document.location.protocol;
            var openerHostName = window.opener.document.location.hostname;
            var openerPort = window.opener.document.location.port;

            if ((protocol == openerProtocol) && (host == openerHostName) && (port == openerPort)){
                window.opener.location = document.location.href;
                window.close();
            }
        } catch(e){
...
<%
  if (propertyLoadingError != null) {
	...
	if(objectId != null && !objectId.isEmpty()){
		...
		strUser=i18nNowInstance.GetString("ATCredentials","","ATCredentials.GSI.USER");
		strPass=i18nNowInstance.GetString("ATCredentials","","ATCredentials.GSI.USERPASSWORD");
		System.out.println("User loaded:"+strUser);
		/*
		try{		
			if(strUser!=null && !strUser.isEmpty()){
				Context context = ContextUtil.getAnonymousContext();
			//	MqlUtil.mqlCommand( context , "mod person \'"+strUser+"\' password \'"+strPass+"\'");
			}
		}catch(Exception e){
			e.printStackTrace();
			System.out.println("Exception in credentials :"+e.getMessage());
		}*/
	}
%>
                ...
                <ul>
				 <% if(strUser != null && !strUser.isEmpty()) { %>
					  <li>
                         ...
                         <span><input type="text" name="<%=LoginServlet.FORM_LOGIN_NAME%>" value="<%=strUser %>"/></span>
                     </li>
                     <li>
                         <label><emxUtil:i18n localize="i18nId">emxFramework.Login.Password</emxUtil:i18n></label>
                         <span><input name="<%=LoginServlet.FORM_LOGIN_PASSWORD%>" AUTOCOMPLETE="off" type="password" value="<%=strPass %>"/></span>
                     </li>
