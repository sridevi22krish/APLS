...
String sSymbolicRelEBOMName   = FrameworkUtil.getAliasForAdmin(context,
	...
	//Modified for QC5116		
	String strMGSRootNodeErrorMsgForCopyTo  = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", 
...
<%
       ...
       //process - relId|objectId|parentId - using the tableRowId
        ...
        <%
            ...
            //    Replace with Existing
                ...
                DomainObject parent = DomainObject.newInstance(context, selPartParentOId);
                String parentType = parent.getInfo(context, DomainConstants.SELECT_TYPE);
                
                DomainRelationship selectRel = new DomainRelationship(selPartRelId);
                ...
                if(parentType.equalsIgnoreCase("AT_C_EXPECTED_PRODUCT") && instanceMaturity.equalsIgnoreCase("RELEASED")){
                	//String strRootNodeErrorMsgForReplaceExisting = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource",	context.getLocale(),"emxEngineeringCentral.BOM.ReplaceExistingRootNodeError");
                    ...
                    <script language="Javascript">
                        if(warningMessage == ""){
                        	//XSSOK
                            warningMessage = "<%=strInstanceMaturiyReleasedwithEPParent%>";
    					}
                    </script>
                    <%
                    break;
 ...
 //Modified for QC5116 
    DomainObject partObj = DomainObject.newInstance(context, selPartObjectId);
    String partObjPolicy = (String) partObj.getInfo(context, DomainConstants.SELECT_POLICY);
		  ...
		  if ("AT_C_LOGICAL_NODE".equals(sTypeObject)) {
        	  sTypeFilterSearch ="type_AT_C_CONFIGURATION_ITEM,type_AT_C_LOGICAL_NODE";
			    ...
			    if(calledMethod.equals("replaceExisting")){
					 sTypeFilterSearch ="type_AT_C_CONFIGURATION_ITEM";
				}else{
					 sTypeFilterSearch ="type_AT_C_EXPECTED_PRODUCT";
				}
          } else if ("AT_C_EXPECTED_PRODUCT".equals(sTypeObject)){
				if(calledMethod.equals("replaceExisting")){
					 sTypeFilterSearch ="type_AT_C_EXPECTED_PRODUCT";
				}else{
					sTypeFilterSearch ="type_AT_C_DESIGN_PART,type_AT_C_COS,type_AT_C_STANDARD_PART";
				}
		  } else if ("AT_C_DESIGN_PART".equals(sTypeObject)) {
        	  sTypeFilterSearch ="type_AT_C_DESIGN_PART,type_AT_C_COS,type_AT_C_STANDARD_PART";
		  }else if ("AT_C_COS".equals(sTypeObject) && calledMethod.equals("replaceExisting")){
			  sTypeFilterSearch ="type_AT_C_DESIGN_PART,type_AT_C_COS,type_AT_C_STANDARD_PART";
		  }else if ("AT_C_STANDARD_PART".equals(sTypeObject) && calledMethod.equals("replaceExisting")){
			  sTypeFilterSearch ="type_AT_C_DESIGN_PART,type_AT_C_COS,type_AT_C_STANDARD_PART";
		  }  else {
	...
	//Modified to fix IR-090294V6R2012
	contentURL ="../common/emxFullSearch.jsp?field=TYPES="+sTypeFilterSearch+":POLICY=policy_ECPart,policy_DevelopmentPart,policy_ConfiguredPart,policy_StandardPart:IS_ASSIGNED_PART=False:CURRENT=policy_ECPart.state_Preliminary,policy_ECPart.state_Review,policy_ECPart.state_Approved,policy_ECPart.state_Release,policy_DevelopmentPart.state_Complete,policy_DevelopmentPart.state_PeerReview,policy_DevelopmentPart.state_Create,policy_ConfiguredPart.state_Preliminary&showInitialResults=false&table=ENCAffectedItemSearchResult&suiteKey="+suiteKey+"&cancelLabel=emxFramework.Command.Cancel&submitLabel=emxFramework.Command.Done&HelpMarker=emxhelpfullsearch&excludeOIDprogram=emxUnresolvedPart:at_excludeRecursiveOIDAddExistingAndTemplate&excludeOID="+selPartObjectId+"&hideHeader=true&objectId="+objectId+"&selection=multiple&selPartObjectId="+selPartObjectId+"&selPartRelId="+selPartRelId+"&relType="+relType+"&selPartParentOId="+selPartParentOId+"&replace=true&submitURL=../engineeringcentral/emxEngrPartBOMHiddenProcess.jsp?calledMethod="+calledMethod;	
	...
	//Modified to fix IR-090294V6R2012
	    	//modified for QC QC5116	
		if(partObjPolicy.equals(EngineeringConstants.POLICY_CONFIGURED_PART)){
			stateNames = "POLICY=policy_ConfiguredPart:CURRENT=policy_ConfiguredPart.state_Release";
		}
	...
	//Modified to fix IR-090294V6R2012
		...
		if(!partObjPolicy.equals(EngineeringConstants.POLICY_CONFIGURED_PART)){
			...
			%>
                <script language="javascript">
                    warningMessage = "<%=strMGSRootNodeErrorMsgForCopyTo%>";
                </script>
            <%
		}
...
var frameName = parent.name;
    if(warningMessage != ""){
        ...
        if(mode == 'changePosition' || mode=='AVLCopyFrom') {
		   ...
		   var cancelReplaceExisting = false;
		   var objType = '<%=sTypeObject%>';
		   
 	       if(mode!='removePart' && mode!='addExisting' && mode!='replaceExisting') {
			...
			if(mode == 'replaceExisting'){
				if (objType == 'AT_C_DESIGN_PART' || objType == 'AT_C_COS' || objType == 'AT_C_STANDARD_PART') {
					if(confirm("<%=strWarningCFReplaceByNewPartMsg%>")) {
					  showModalDialog("../common/emxBlank.jsp","570","570","true"); 	          
					  var objWindow =  getTopWindow().modalDialog.contentWindow;
					  document.engrfullsearch.target=objWindow.name;
					}else{
						cancelReplaceExisting = true;
					}
				}else{
					showModalDialog("../common/emxBlank.jsp","570","570","true"); 	          
					var objWindow =  getTopWindow().modalDialog.contentWindow;
 	      	...
 	      	if(mode=='addExisting') {
 	      		if((!getTopWindow().getWindowOpener()) && (sCustomFilter == "engineering" || sCustomFilter == "Engineering") && frameName == "ENCBOM"){
	 	      		...
	 	      		var tablemode = "view";
	 	      		if(contentFrame.editableTable && contentFrame.editableTable != null && contentFrame.editableTable != undefined){
		      ...
		      //XSSOK
				...
				//XSSOK
				if(cancelReplaceExisting == false) {
					...
					if("view"== tablemode && warningMessageinViewMode != ""){
					  ...
					  var objWindow =  getTopWindow().modalDialog.contentWindow;
					  document.engrfullsearch.target=objWindow.name;
					  document.engrfullsearch.action="<%=contentURL%>"+"&tablemode="+tablemode;
					  document.engrfullsearch.submit();
					}
				}
	        }
